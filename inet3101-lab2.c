//Leng Thao - Lab 2: Memory and Structure
...
void addRecord(struct part **database, int *numRecords) {
    ...
    *numRecords = newCount;
    ...
    //Start gathering data from the user for the new record
...
int main(void) {
    struct part *database = NULL; // Pointer to our dynamic array of Part structs
    int numRecords = 0; // Number of records in our database
    int choice = 0; //User choice
    int running = 1; //Runs the program until it's over
    ...
    // Clean up any remaining allocated memory before exiting
    free(database);
    database = NULL;
